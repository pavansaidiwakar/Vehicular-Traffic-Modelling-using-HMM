...
class Model(object):
    ...
    def evaluate(self, sequence):
        ...
        prob = 0
	predicted_symbol = 'RRRR'
	predicted_state = '1'
        alpha,index = self._forward(sequence)
	maximum = 0
        for state_to in self._states:
            prob = 0
            for state_from in self._states:
                prob += alpha[index - 1][state_from] * \
                    self.trans_prob(state_from, state_to)
	    for symbols in self._symbols:	
                if(prob * self.emit_prob(state_to, symbols) > maximum):
		    maximum = prob * self.emit_prob(state_to, symbols)
		    predicted_symbol = symbols
		    predicted_state = state_to
        return (predicted_state,predicted_symbol)
